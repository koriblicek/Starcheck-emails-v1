...
export function DesktopItemColumn({ column, isContainerSelected, containerIndex, columnIndex }: IDesktopItemColumnProps) {
    ...
    const blocks = column.blocks.map((block, index) => {
        return <DesktopItemBlock block={block} key={block.id} isContainerSelected={isContainerSelected} containerIndex={containerIndex} columnIndex={columnIndex} blockIndex={index + 1} columnPadding={ column.padding.value} />;
    });
